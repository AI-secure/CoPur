...
import numpy as np
import copy 
from utils.nus_utils import get_local_outputs,load_data, load_models
...
def parse_command():
    ...
    parser.add_argument('--attackers_index', nargs='+', type=int, default=[3])
    parser.add_argument('--use_ae', action='store_true')
    parser.add_argument('--purify_epochs', type=int, default=3)
    ...
    parser.add_argument('--PGDeps', type=float, default=0.1)
    parser.add_argument('--PGDiters', type=int, default=50)
    ...
    ######args.num_text_clients
    args.observe_list = list(set(list(range(0, args.num_clients))) - set(args.miss_index))
...
(test_images, test_texts), y_test = load_data(args, test=True)
#print(y_test[0:100,])
...
test_accuracy1 = tf.keras.metrics.CategoricalAccuracy(name='test_accuracy1')

test_loss2 = tf.keras.metrics.Mean(name='test_loss2')
test_accuracy2 = tf.keras.metrics.CategoricalAccuracy(name='test_accuracy2')

test_loss3 = tf.keras.metrics.Mean(name='test_loss3')
test_accuracy3 = tf.keras.metrics.CategoricalAccuracy(name='test_accuracy3')
...
def RandomizedSmooth(args, LocalOutputs, active_model,batch_size=100):
    counts = np.zeros(args.num_class, dtype=int)
    ...
    for _ in range(ceil(num / batch_size)):
        ...
        predictions1=tf.Variable(predictions).numpy()
        predictions1=predictions1.argmax(axis=1)
        counts += count_arr(predictions1, args.num_class)
...
def create_adversarial_pattern(_LocalOutputs, args, labels, target_labels, server_model, PGD=True):
    loss_object = tf.keras.losses.CategoricalCrossentropy()
    if PGD == True:
        ...
        for i in range(args.PGDiters):
            ...
            with tf.GradientTape() as tape:
                ...
                if args.targeted == 1:
                    loss = loss_object(target_labels, output)  # targeted attack
                else:
                    loss = loss_object(labels, output)  # untargeted attack
...
def test(_LocalModels,_active_model ):
    ...
    test_ds = tf.data.Dataset.from_tensor_slices(
        ((test_images, test_texts), y_test)).batch(args.batch_size)
    count_index = 0
    for (images, texts), labels in test_ds:
        count_index += 1
        print('Sample index:', count_index)
        
        _LocalOutputs= get_local_outputs(args.num_img_clients, images, args.img_feature_div, 
                     args.num_text_clients, texts, args.text_feature_div, _LocalModels)
        ...
        if args.targeted==1:
            _LocalOutputs = create_adversarial_pattern(_LocalOutputs, args, labels, target_labels, _active_model, PGD=True)
        else:
            for j in range(args.num_attacker):
                atk_index = args.attackers_index[j]
        ...
        if args.use_ae:
            LocalOutputs = inference_purify_miss(args, _LocalOutputs,autoencoder_model,L_optimizer,L_optimizer2,rae_loss_object, vis=False)#LocalOutputs = inference_purify(args, _LocalOutputs,autoencoder_model,L_optimizer,L_optimizer2,rae_loss_object, vis=True)
        else:
            LocalOutputs = _LocalOutputs
        ...
        test_accuracy3(labels,tf.one_hot(RandomizedSmooth(args, LocalOutputs, _active_model,100),5))



target_labels = tf.constant([0, 1, 0, 0, 0], dtype=tf.float32, shape=(1, 5))
